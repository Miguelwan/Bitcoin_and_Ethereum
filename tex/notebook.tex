...
\end{Verbatim}
    ...
    \subsection{Exploration of Bitcoin vs Ethereum}\label{exploration-of-bitcoin-vs-ethereum}

As we could see in the previous images, Bitcoin and Ethereum have
similarities in the behavior of their value. The most notorious are the
months of February, October, and September. To explore this similarities
we should look at different indexes.
...
of these two assets, we should plot the variation of adjustment of both
...
\end{Verbatim}
    ...
    In principle, from the previous graphic we could conclude that our
...
graphic and no correlation between the two currencies.
...
R[write to console]: `geom\_smooth()` using formula 'y \textasciitilde{} x'
    ...
    \subsection{Exploration of Ethereum vs Basic Attention Token}\label{ethereum-vs-basic-attention-token}

As we could see in the previous images, Ethereum and the Basic Attention
Token have no many similarities in the behavior of their value. We can
also see that the months of February and October show some similarity. To
...
cannot see more similarities. We should plot the variation of adjustment
...
Token in the graphic and no correlation between the two currencies.
...
\end{Verbatim}
    ...
    \section{The correlation between Bitcoin vs Ethereum}\label{the-correlation-between-bitcoin-vs-ethereum}
...
this in order to understand if the behavior exposed by Bitcoin and
Ethereum during 2020 was a singularity of that time interval, or if the
value of both assets are linearly related in general via the market cap
and traders.

    \subsection{Building and testing the model}\label{buulding-and-testing-the-model}
...
R[write to console]: `geom\_smooth()` using formula 'y \textasciitilde{} x'
    ...
    \subsection{Predicting new data (January 2021)}\label{predicting-new-data-january-2021}
...
showing the Japanese Candlesticks before we apply our model, this
...
\end{Verbatim}
    ...
    From the previous images it is not clear if there is a correlation
between these two volumes, in the first part of 2020 the volumes are
clearly not correlated, but in the second part the volumes look similar.
